...
<script type='text/javascript'>
    function VerifieSuppression(Formulaire, IdfElement) {
        ...
        if (chaine == "") {
            alert("Pas de suivi s�lectionn�");
        } else {
            Message = "Etes-vous s�r de supprimer ces suivis :\n" + chaine + "?";
            if (confirm(Message)) {
    ...
    function VerifieChamps(Formulaire) {
        ...
        if (fourchette_envoyee == "") {
            ListeErreurs += "La fourchette envoy�e est obligatoire\n";
        }
        if (!Verifie_Fourchette(fourchette, fourchette_envoyee)) {
            ListeErreurs += "La fourchette envoy�e d�borde de la fourchette de base\n";
        }
        if (envoi_adherent == "") {
            ListeErreurs += "La date de l'envoi au releveur est obligatoire\n";
        }
        if (!date_ptn.test(envoi_adherent)) {
            ListeErreurs += "La date de l'envoi au releveur doit �tre de la forme : JJ/MM/AAAA\n";
        }
...
function menu_liste($sconnexionBD)
{
    ...
    print("<br><div align=center><input type=button value=\"Supprimer les suivis s�lectionn�s\" ONCLICK=VerifieSuppression(0,\"supp[]\")></div>");
    print("</form>");
 ...
 * @param string $pst_fourchette_envoyee Fourchette des photos envoy�e
 ...
 * @param array $pa_adherents liste des releveurs (adh�rents) 
 */
function menu_edition($pi_id_adherent, $pi_id_commune, $pst_fourchette, $pi_id_collection, $pst_fourchette_envoyee, $pst_envoi_adherent, $pst_retour_adherent, $pa_communes, $pa_collections, $pa_adherents)
{
    ...
    print("<tr><th>Fourchette envoy�e (aaaa-aaaa)</th><td><input type=\"text\" maxsize=9 size=9 name=fourchette_envoi value=\"$pst_fourchette_envoyee\"></td></tr>");
 ...
 * @param integer $pi_idf_suivi Identifiant du relev�
 * @param array $pa_communes Liste des commmunes
 * @param array $pa_collections Liste des collections
 * @param array $pa_adherents Liste des adh�rents
 */
