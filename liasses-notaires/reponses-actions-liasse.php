...
print('<form action="recherches-actions-liasses.php" method="post">');
...
if ($i_nb_ligne_extr > 0) {
	...
	if ($_SESSION['menu_rla'] == 'publication') {
		...
		$pagination = new PaginationTableau(
			...
			DELTA_NAVIGATION,
			array(
				'Cote', 'Notaire(commune)', 'Période', 'Forme de liasse', 'Intervenant', 'Priorité',
		...
		foreach ($a_liasses as $a_liasse) {
			list(
				$st_cote_liasse, $st_libelle_notaires, $st_libelle_annees, $st_forme, $st_consult, $st_releveur, $st_publi_pap,
		...
		$pagination = new PaginationTableau(
			...
			DELTA_NAVIGATION,
			array(
				'Cote', 'Notaire(commune)', 'Période', 'Forme de liasse', 'Consultable', 'Releveur', 'Papier',
		...
		foreach ($a_liasses as $a_liasse) {
			list(
				$st_cote_liasse, $st_libelle_notaires, $st_libelle_annees, $st_forme, $st_consult, $st_publi_pap, $st_publi_num,
		...
		foreach ($a_liasses as $a_liasse) {
			list(
				$st_cote_liasse, $st_libelle_notaires, $st_libelle_annees, $st_forme, $st_consult,
				$st_photographe, $st_date_photo, $st_couverture_photo, $st_codif_photo
			) = $a_liasse;
			$a_tableau[] = array($st_cote_liasse, $st_libelle_notaires, $st_libelle_annees, $st_forme, $st_consult);
		}
