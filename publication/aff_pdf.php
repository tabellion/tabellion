...
class PDF extends FPDF
{
	...
	function Footer()
	{
		if ($this->PageNo() == 1) { //Premi�re page
		} else {   //Pages suivantes
...
switch ($type_actes_nimegue) {
	...
	case "R": //selection sur les divers
		$titre = "Recensements";
		$titreHP = $titre . " de " . $commune;
		$sql = "SELECT * FROM tmp_publication ORDER BY `data6`,`data7`,`data10`,`data11` ASC"; // tri sur le ann�e page maison m�nage
		$titre3 = "Par ann�e, maison,m�nage";
		break;
}
	...
	switch ($type_actes_nimegue) {
		...
		case "N": // $type_actes_nimegue = N
			...
			if (empty($data[14])) {
				...
				$l1 = $l1 . "  - " . "P�re   " . $data[14] . "   " . $data[15] . "   " . $data[16] . "   " . $data[17] . "\n";
			}
			if (empty($data[18])) {
			} else {
				$l1 = $l1 . "  - " . "M�re   " . $data[18] . "   " . $data[19] . "   " . $data[20] . "   " . $data[21] . "\n";
			}
			if (empty($data[22])) {
			} else {
				$l1 = $l1 . "  - " . "Par/T�m1   " . $data[22] . "   " . $data[23] . "   " . $data[24] . "\n";
			}
			if (empty($data[25])) {
			} else {
				$l1 = $l1 . "  - " . "Par/T�m2   " . $data[25] . "   " . $data[26] . "   " . $data[27] . "\n";
			}
			if (empty($data[28])) {
			} else {
				$l1 = $l1 . $data[28] . "\n";
		...
		case "D": // $type_actes_nimegue = D
			...
			if (empty($data[2])) {
				...
				$lieuorigine = "lieu d origine " . $data[12];
			}  //$lieuorigine = $data[data12]
			if ($data[13] !== "") {
				$datenaiss = "n�(e) le : " . $data[13];
			} else {
				$datenaiss = "";
			} //$datenaiss = $data[data13]
			if (empty($data[15])) {
				$ages = "";
			} else {
				$ages = " Age : " . $data[15] . "";
			...
			if (empty($data[18])) {
				...
				$l1 = $l1 . "  - " . "Conjoint   " . $data[18] . "   " . $data[19] . "   " . $data[20] . "   " . $data[21] . "\n";
			}
			if (empty($data[22])) {
			} else {
				$l1 = $l1 . "  - " . "P�re   " . $data[22] . "   " . $data[23] . "   " . $data[24] . "   " . $data[25] . "\n";
			}
			if (empty($data[26])) {
			} else {
				$l1 = $l1 . "  - " . "M�re   " . $data[26] . "   " . $data[27] . "   " . $data[28] . "   " . $data[29] . "\n";
			}
			if (empty($data[30])) {
			} else {
				$l1 = $l1 . "  - " . "T�m1   " . $data[30] . "   " . $data[31] . "   " . $data[32] . "\n";
			}
			if (empty($data[33])) {
			} else {
				$l1 = $l1 . "  - " . "T�m2   " . $data[33] . "   " . $data[34] . "   " . $data[35] . "\n";
			}
			if (empty($data[36])) {
			} else {
				$l1 = $l1 . $data[36] . "\n";
		...
		case "M": // $type_actes_nimegue = M
			...
			if (empty($data[12])) {
				...
				$lieuorigine1 = " Originaire de " . $data[12] . " ";
			}  //$lieuorigine 
			//if ($data[data13]!=''or 0) {$datenaiss1 = "";} else {$datenaiss1 = " n� le : ".$data[data13]." ";}
			if ($data[13] != '' or empty($data[13])) {
				$datenaiss1 = "";
			} else {
				$datenaiss1 = " n� le : " . $data[13] . " ";
			} //$datenaiss
			if (empty($data[14])) {
				$ages1 = "";
			} else {
				$ages1 = " Age : " . $data[14] . " ans ";
			} //$ages 
			$info1 = $lieuorigine1 . $datenaiss1 . $ages1; //Lieu origine + date naiss + age
			if (empty($data[16])) {
				$prof = "";
			} else {
				$prof = " Profession " . $data[16] . " ";
			}  //Profession
			if (empty($data[15])) {
				$commentaire1 = "";
			} else {
			...
			if (empty($commentaireEpx)) {
			} else {
				$l1 = $l1 . "   - " . $commentaireEpx . "\n";
			}
			if (empty($data[17])) {
			} else {
				$l1 = $l1 . "   - " . "Veuf de : " . $data[17] . " " . $data[18] . " " . $data[19] . "\n";
			}
			if (empty($data[20])) {
			} else {
				$l1 = $l1 . "   - " . "P�re  : " . $data[20] . " " . $data[21] . " " . $data[22] . " " . $data[23] . "\n ";
			}
			if (empty($data[24])) {
			} else {
				$l1 = $l1 . "   - " . "M�re  : " . $data[24] . " " . $data[25] . " " . $data[26] . " " . $data[27] . " \n";
			}
			...
			if (empty($data[30])) {
				...
				$lieuorigine2 = " Originaire de " . $data[30] . " ";
			}  //$lieuorigine
			if ($data[31] != '' or empty($data[31])) {
				$datenaiss2 = "";
			} else {
				$datenaiss2 = " n�e le : " . $data[31] . " ";
			} //$datenaiss 
			if (empty($data[32])) {
				$ages2 = "";
			} else {
				$ages2 = " Age : " . $data[32] . " ans ";
			} //$ages 
			$info2 = $lieuorigine2 . $datenaiss2 . $ages2; //Lieu origine + date naiss + age
			if (empty($data[34])) {
				$prof2 = "";
			} else {
				$prof = " Profession " . $data[34] . " ";
			}  //Profession
			if (empty($data[33])) {
				$commentaire2 = "";
			} else {
				$commentaire2 = " " . $data[33] . " ";
			}  //commentaire2
			...
			if (empty($commentaireEp)) {
			} else {
				$l1 = $l1 . "   - " . $commentaireEp . "\n";
			}
			if (empty($data[35])) {
			} else {
				$l1 = $l1 . "   - " . "Veuve de : " . $data[35] . " " . $data[36] . " " . $data[37] . "\n";
			}
			if (empty($data[38])) {
			} else {
				$l1 = $l1 . "   - " . "P�re  : " . $data[38] . " " . $data[39] . " " . $data[40] . " " . $data[41] . "\n";
			}
			if (empty($data[42])) {
			} else {
				$l1 = $l1 . "   - " . "M�re  : " . $data[42] . " " . $data[43] . " " . $data[44] . " " . $data[45] . "\n";
			}
			// T�moins
			if (empty($data[46])) {
			} else {
				$l1 = $l1 . "    - " . "T�moin 1  : " . $data[46] . " " . $data[47] . " " . $data[48] . "\n ";
			}
			if (empty($data[49])) {
			} else {
				$l1 = $l1 . "    - " . "T�moin 2  : " . $data[49] . " " . $data[50] . " " . $data[51] . "\n ";
			}
			if (empty($data[52])) {
			} else {
				$l1 = $l1 . "    - " . "T�moin 3  : " . $data[52] . " " . $data[53] . " " . $data[54] . "\n ";
			}
			if (empty($data[55])) {
			} else {
				$l1 = $l1 . "    - " . "T�moin 4  : " . $data[55] . " " . $data[56] . " " . $data[57] . "\n ";
			}
			if (empty($data[58])) {
			} else {
				$l1 = $l1 . "    - " . $data[58] . "\n";
		...
		case "V": // $type_actes_nimegue = V
			...
			if (empty($data[15])) {
				...
				$lieuorigine1 = "Originaire de " . $data[15] . " ";
			}  //$lieuorigine 
			if ($data[16] != '' or empty($data[16])) {
				$datenaiss1 = "";
			} else {
				$datenaiss1 = "n� le : " . $data[16] . " ";
			} //$datenaiss 
			if (empty($data[17])) {
				$ages1 = "";
			} else {
				$ages1 = "Age : " . $data[17] . " ans ";
			} //$ages 
			$info1 = $lieuorigine1 . $datenaiss1 . $ages1; //Lieu origine + date naiss + age
			if (empty($data[19])) {
				$prof = "";
			} else {
				$prof = "Profession " . $data[19] . " ";
			}  //Profession
			if (empty($data[18])) {
				$commentaire1 = "";
			} else {
				$commentaire1 = " " . $data[18] . " ";
			}  //commentaire1
			$commentaireEpx = $prof . $commentaire;
			...
			if (empty($data[20])) {
				...
				$l1 = $l1 . "  - " . "Ex �pouse : " . $data[20] . " " . $data[21] . " " . $data[22] . "\n";
			}
			if (empty($data[23])) {
			} else {
				$l1 = $l1 . "  - " . "P�re  : " . $data[23] . " " . $data[24] . " " . $data[25] . " " . $data[26] . "\n";
			}
			if (empty($data[27])) {
			} else {
				$l1 = $l1 . "  - " . "M�re  : " . $data[27] . " " . $data[28] . " " . $data[29] . " " . $data[30] . "\n";
			}
			...
			if (empty($data[34])) {
				...
				$lieuorigine2 = "Originaire de " . $data[34] . " ";
			}  //$lieuorigine
			if ($data[35] != '' or empty($data[35])) {
				$datenaiss2 = "";
			} else {
				$datenaiss2 = "n�e le : " . $data[35] . " ";
			} //$datenaiss 
			if (empty($data[36])) {
				$ages2 = "";
			} else {
				$ages2 = "Age : " . $data[36] . " ans ";
			} //$ages 
			$info2 = $lieuorigine2 . $datenaiss2 . $ages2; //Lieu origine + date naiss + age
			if (empty($data[38])) {
				$prof2 = "";
			} else {
				$prof = "Profession " . $data[38] . " ";
			}  //Profession
			if (empty($data[37])) {
				$commentaire2 = "";
			} else {
				$commentaire2 = " " . $data[37] . " ";
			}  //commentaire2
			$commentaireEp = $prof2 . $commentaire2;
			...
			if (empty($data[39])) {
				...
				$l1 = $l1 . "  - " . "Ex �poux : " . $data[39] . " " . $data[40] . " " . $data[41] . "\n";
			}
			if (empty($data[42])) {
			} else {
				$l1 = $l1 . "  - " . "P�re  : " . $data[42] . " " . $data[43] . " " . $data[45] . " " . $data[44] . "\n";
			}
			if (empty($data[46])) {
			} else {
				$l1 = $l1 . "  - " . "M�re  : " . $data[46] . " " . $data[47] . " " . $data[49] . " " . $data[48] . "\n";
			}
			...
			if (empty($data[53])) {
				...
				$l1 = $l1 . "  - " . "T�moin 2  : " . $data[53] . " " . $data[54] . "  " . $data[55] . "\n";
			}
			if (empty($data[56])) {
			} else {
				$l1 = $l1 . "  - " . "T�moin 3  : " . $data[56] . " " . $data[57] . "  " . $data[58] . "\n";
			}
			if (empty($data[59])) {
			} else {
				$l1 = $l1 . "  - " . "T�moin 4  : " . $data[59] . " " . $data[60] . "  " . $data[61] . "\n";
			}
		...
		case "R": // $type_actes_nimegue = R pour recensementt
			...
			//$pdf->Cell(50,3,"Ann�e : ".$data[6]." Quartier : ".$data[8]." Rue : ".$data[9]." N� de maison : " .$data[10]." N� M�nage : ",0,0,L);
			//$pdf->Cell(20,3,$data[11]." ".$data[12]." ".$data[13]."Age : ".$data[14]." "."Ann�e naissance :".$data[15]." Profession :".$data[16],0,0,L);
			//
			$l1 = '';
			$l1 = $l1 . "Ann�e : " . $data[6] . " - Quartier : " . $data[8] . " - Rue : " . $data[9] . " - N� de maison : " . $data[10] . " - N� M�nage : " . $data[11] . "\n";
			$l1 = $l1 . "-" . $data[12] . " - " . $data[13] . " - " . $data[14] . " - Age : " . $data[15] . " - n� en :" . $data[16] . " � " . $data[17] . " - Profession :" . $data[18] . "\n";
