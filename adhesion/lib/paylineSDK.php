	...
	**/
	...
	static function responseToArrayForGetCards($response){
		...
		foreach($response as $k=>$v){
			...
			if (is_object($v) && ($k != 'cards' ) )  {$array[$k] = util::responseToArrayForGetCards($v); }
			else { 
...
class paylineSDK{
	...
	**/
	function __construct() {
		...
		$this->header_soap['version'] = "kit PHP version 1.1.10";
	...
	**/
	 ...
	 * payment, returnURL, cancelURL, order, notificationURL,customPaymentTemplateURL, contracts, secondContracts
	...
	**/
	public function do_webpayment($array,$debug=0) {
			...
			if($array && is_array($array)){
				...
				if(!isset($array['secondContracts'])||!strlen($array['secondContracts'][0]))$array['secondContracts'] = explode(";", SECOND_CONTRACT_NUMBER_LIST);
 ...
 * @params : array : array. the array keys are :
 * @return : Array. Array from a payline server response object.
 * @description : return current encryption key for card encryption
**/
 ...
 * function getBalance
 * @params : array : array. the array keys are :
 * cardID, contractNumber
 * @return : Array. Array from a payline server response object.
 * @description : return the available amount of a pre-paid card
**/
...
public function transactionsSearch($array){
	...
	public function SwitchToPrimary(){
		...
		$this->header_soap['connection_timeout'] = $this->CALL_TIMEOUT;
		if(PRODUCTION){
			$this->WSDL_SOAP = WSDL_PATH.'production/WebPaymentAPI.wsdl'; 
			$this->WSDL_DIRECT_SOAP = WSDL_PATH.'production/DirectPaymentAPI.wsdl';
			$this->WSDL_EXTENDED_SOAP = WSDL_PATH.'production/ExtendedAPI.wsdl';
		}else{
			$this->WSDL_SOAP = WSDL_PATH.'homologation/WebPaymentAPI.wsdl'; 
			$this->WSDL_DIRECT_SOAP = WSDL_PATH.'homologation/DirectPaymentAPI.wsdl';
			$this->WSDL_EXTENDED_SOAP = WSDL_PATH.'homologation/ExtendedAPI.wsdl';
		}
	}
	...
	public function SwitchToSecondary(){
		...
		$this->header_soap['connection_timeout'] = $this->CALL_TIMEOUT;
		if(PRODUCTION){
			$this->WSDL_SOAP = WSDL_PATH.'productionHD/WebPaymentAPI.wsdl'; 
			$this->WSDL_DIRECT_SOAP = WSDL_PATH.'productionHD/DirectPaymentAPI.wsdl';
			$this->WSDL_EXTENDED_SOAP = WSDL_PATH.'productionHD/ExtendedAPI.wsdl';
		}else{
			$this->WSDL_SOAP = WSDL_PATH.'homologationHD/WebPaymentAPI.wsdl'; 
			$this->WSDL_DIRECT_SOAP = WSDL_PATH.'homologationHD/DirectPaymentAPI.wsdl';
			$this->WSDL_EXTENDED_SOAP = WSDL_PATH.'homologationHD/ExtendedAPI.wsdl';
		}
	}	
	...
	public function SwitcherFormatResponse($DateDebut,$Method,$WS,$WSRequest,$WDSL){
		$DateFin = time();
		$this->SetDefaultSocketTimeOut();
		$response = array();
		while($this->NMAX_TENTATIVE >= $this->CURRENT_NUMBER_CALL){
