...
function menu_liste($pconnexionBD) {					
	...
	{        
		$pagination = new PaginationTableau(basename(filter_input(INPUT_SERVER, 'SCRIPT_NAME', FILTER_SANITIZE_STRING)),'num_page',$i_nb_liasses,
		                                    10,DELTA_NAVIGATION,array('Cote','Libellé','Notaire(s)','Periode(s)','Modifier','Supprimer'));		
...
function menu_edition($pst_cote, $pst_libelle, $pst_periodes, $pst_notaires, $pi_depose_ad, 
                      $pst_idf_dept_depose_ad, $pa_depts_depose_ad, $pi_liasse_consult, 
					  $pi_idf_forme_liasse, $pa_formes_liasses, $pst_info_compl, $pst_mode){	
...
function menu_modifier($pconnexionBD, $pst_cote_liasse, $pa_depts_depose_ad, $pa_formes_liasses) {	/*	Affiche le menu de modification d'une liasse
...
function menu_ajouter($pconnexionBD, $pa_depts_depose_ad, $pa_formes_liasses){		/** Affiche le menu d'ajout d'une liasse
															* @param array		$pa_depts_depose_ad		Tableau des départements de dépose AD 
															* @param array		$pa_formes_liasses		Tableau des formes de liasses 			*/ 
