...
if ($_SESSION['menu_rla'] != 'publication')
	$csv .= $_SESSION['pdf']['nb_liasse'] . " liasses;" . $_SESSION['pdf']['pourc_liste'] . " % de la liste;" . $_SESSION['pdf']['pourc_tot'] . " % de la s�rie\n";

switch ($_SESSION['menu_rla']) {
	case 'publication':
		$csv .= "Titre publication papier;Date;Informations compl�mentaires\n";
		...
		break;
	case 'publi_pap':
		$csv .= "Titre publication papier;Date;Cote;Notaire(commune);P�riode;Forme liasse\n";
		foreach ($a_liasses as $a_liasse) {
		...
		break;
	case 'program':
		$csv .= "Cote;Notaire(commune);P�riode;Forme liasse;Intervenant;Priorit�;Ech�ance;Prog. relev�;Prog. photo\n";
		foreach ($a_liasses as $a_liasse) {
			list(
				$st_cote_liasse, $st_libelle_notaires, $st_libelle_annees, $st_forme, $st_intervenant, $st_priorite,
		...
		break;
	case 'releve':
		$csv .= "Cote;Notaire(commune);P�riode;Forme liasse;Consultable;Releveur;Papier;Num�rique;Date relev�;Infos relev�\n";
		foreach ($a_liasses as $a_liasse) {
			list(
				$st_cote_liasse, $st_libelle_notaires, $st_libelle_annees, $st_forme, $st_consult, $st_releveur, $st_publi_pap, $st_publi_num,
				$st_date_fin_releve, $st_info_compl
			) = $a_liasse;
			$st_libelle_notaires = str_replace(";", "-", $st_libelle_notaires);
			$st_libelle_annees = str_replace(";", "-", $st_libelle_annees);
			$st_info_compl = str_replace(";", "-", $st_info_compl);
		...
		break;
	case 'complete':
		$csv .= "Cote;Notaire;Commune;P�riode;Forme liasse;Consultable;Infos liasse;Relev�;Infos relev�\n";
		foreach ($a_liasses as $a_liasse) {
		...
		break;
	case 'publi_num':
		$csv .= "Cote;Notaire(commune);P�riode;Forme liasse;Consultable;Releveur;Date relev�\n";
		foreach ($a_liasses as $a_liasse) {
	...
	case 'photo':
		if ($_SESSION['avec_commentaire_rla'] != 'oui') {
			$csv .= "Cote;Notaire(commune);P�riode;Forme liasse;Consultable;Papier;Num�rique;Photographe;Date photo;Couverture;Codification\n";
			foreach ($a_liasses as $a_liasse) {
			...
			$csv .= "Cote;Notaire(commune);P�riode;Forme liasse;Couverture;Commentaires\n";
			foreach ($a_liasses as $a_liasse) {
		...
		break;
	case 'pas_releve':
		$csv .= "Cote;Notaire(commune);P�riode;Forme liasse\n";
		foreach ($a_liasses as $a_liasse) {
