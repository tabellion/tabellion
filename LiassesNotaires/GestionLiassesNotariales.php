...
$("#idf_commune").select2({allowClear: true,placeholder: ""});
$("#dept_depose_ad").select2({allowClear: true,placeholder: ""});
$("#forme_liasse").select2({allowClear: true,placeholder: ""});
...
$("#cre_groupe_liasses").validate({
  rules: {
		...
		dept_depose_ad:	{ dept_avec_depose:true },
		nom:			{ required:true },
		idf_commune:	{ commune_requise:true }
  },		
  messages: {
		...
		depose_ad:		{ depose_avec_dept: "Le département doit être renseigné pour une liasse déposée aux AD"	},
		dept_depose_ad:	{ dept_avec_depose: "La case 'Déposée aux AD' doit être cochée quand le département est renseigné"	},                                                                                              
		nom:			{ required: "Le nom du notaire est obligatoire"	},
		idf_commune:	{ commune_requise: "La commune de l'étude est obligatoire"	}
  }
});
...
$("#cor_groupe_liasses").validate({
  rules: {
		numeros:		{ groupe_cotes:true, required:true	 },
		idf_commune:	{ nom_prenom_commune:true }
  },		
  messages: {
		numeros:		{ groupe_cotes: "La liste des numéros de liasses doit être composée de n1-n2 ou n1,n2,n3,... ou une combinaison des deux",
						  required: "La liste des numéros de liasses est obligatoire"},
		idf_commune:	{ nom_prenom_commune: "Indiquer au moins une information à corriger"	}
  }
});
...
switch ($gst_mode) {
	...
	case 'VERIFIER_GROUPE' : {
		...
		menu_confirmer_correction_groupe($connexionBD, $a_communes, $_SESSION['groupe']['cotes'], $_SESSION['groupe']['numeros'], 
							$_SESSION['groupe']['nom'], $_SESSION['groupe']['prenom'], $_SESSION['groupe']['commune'], $_SESSION['groupe']['commentaire']);
		break;
	}
