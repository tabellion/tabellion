...
$gi_idf_commune = $_POST['idf_commune'] ?? 1;
$gi_idf_type_acte = $_POST['idf_type_acte'] ?? null;
$gi_idf_type_acte_nimegue = $_POST['idf_type_acte_nimegue'] ?? 0;
$gst_mode = $_POST['mode'] ?? 'SELECTION_NOTIFICATION';
...
function AfficheSelectionNotification($pconnexionBD, $pi_idf_source, $pi_idf_commune, $pi_idf_type_acte)
{
    ...
    print('<div class="panel-heading">Notification du chargement des données d\'une commune/paroisse</div>');
...
function AfficheEditionNotification($pconnexionBD, $pi_idf_source, $pi_idf_commune, $pi_idf_type_acte)
{
    ...
    $st_texte .= "Nous vous informons que la base " . SIGLE_ASSO . " a été mise &agrave; jour:<br /><br />";
    if ($pi_idf_type_acte == IDF_MARIAGE || $pi_idf_type_acte == IDF_NAISSANCE || $pi_idf_type_acte == IDF_DECES) {
        ...
        $st_texte .= "La source '$st_source' de la commune/paroisse <b>" . cp1252_vers_utf8($st_commune) . "</b> " . cp1252_vers_utf8($st_libelle_canton) . " a été mise &agrave; jour sur la période: ";
        ...
        $st_texte .= "Le détail des années disponibles est consultable en cliquant sur l'intervalle des années sur-ligné en bleu ci-dessus.<br /><br />";
        $st_texte .= "La commune comporte désormais $i_nb_actes actes de type: " . cp1252_vers_utf8($st_type_acte) . "<br />";
    } else {
        ...
        $st_texte .= "<table border=1><tr><th>Type d'acte</th><th>Années</th><th>Nombre d'actes</th>";
        ...
        $st_texte .= "</table><br />";
        $st_texte .= "Le détail des années disponibles est consultable en cliquant sur les intervalle des années sur-lignés en bleu ci-dessus<br /><br />";
    }
    $st_texte .= "Pour rappel, la liste complète des dépouillements se trouve &agrave; l'adresse: <a href=\"$gst_url_site/AfficheStatsCommune.php\">$gst_url_site/AfficheStatsCommune.php</a><br /><br />";
 ...
 * @global string $gst_url_site adresse du site  
 */
function EnvoieNotification($pconnexionBD, $pi_idf_commune, $pi_idf_type_acte, $msg_html)
{
    global $user, $gst_rep_site, $gst_serveur_smtp, $gst_utilisateur_smtp, $gst_mdp_smtp, $gi_port_smtp;
    ...
    switch ($pi_idf_type_acte) {
        ...
        case IDF_NAISSANCE:
        case IDF_DECES:
            $sujet = "MAJ Base " . SIGLE_ASSO . ": " . cp1252_vers_utf8($st_type_acte) . " de la commune de " . cp1252_vers_utf8($st_commune);
            break;
        default:
            $sujet = "MAJ Base " . SIGLE_ASSO . ": Actes divers de la commune de " . cp1252_vers_utf8($st_commune);
    }
...
tr:nth-child(even) {background-color: #f2f2f2;}
    ...
    $msg_texte = html_entity_decode(str_ireplace(array("<br>", "<br />", "<hr />", "<hr>"), "\r\n", $msg_html), ENT_COMPAT, 'UTF-8');
    $courriel = new Courriel($gst_rep_site, $gst_serveur_smtp, $gst_utilisateur_smtp, $gst_mdp_smtp, $gi_port_smtp);
    $courriel->setExpediteur($user['email_perso'], $user['prenom'] .' '. $user['nom']);
    $courriel->setDestinataire(EMAIL_FORUM, "Forum " . SIGLE_ASSO);
    $courriel->setSujet($sujet);
    $courriel->setTexte($msg_html);
    $courriel->setTexteBrut($msg_texte);
    if ($courriel->envoie())
        print('<div class="alert alert-success">Notification envoyée avec succès sur le forum</div>');
    else
