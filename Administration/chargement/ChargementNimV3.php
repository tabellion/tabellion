		...
		while (!feof($pf)) {
			...
			if ($b_acte_nexistepas) {
				...
				if ($st_nom_excjt_intv1 != '') {
					...
					switch ($c_sexe_intv1) {
						case 'M':
							$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, $st_type_acte, $intv1->getIdf(), $st_nom_intv1, $cjt_intv1->getIdf(), $st_nom_excjt_intv1);
							break;
						case 'F':
							$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, $st_type_acte, $cjt_intv1->getIdf(), $st_nom_excjt_intv1, $intv1->getIdf(), $st_nom_intv1);
							break;
						default:
							$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, $st_type_acte, $intv1->getIdf(), $st_nom_intv1, $cjt_intv1->getIdf(), $st_nom_excjt_intv1);
					}
				...
				if ($st_nom_intv2 != '') {
					...
					if ($st_nom_excjt_intv2 != '') {
						...
						switch ($c_sexe_intv2) {
							case 'M':
								$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, $st_type_acte, $intv2->getIdf(), $st_nom_intv2, $cjt_intv2->getIdf(), $st_nom_excjt_intv2);
								break;
							case 'F':
								$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, $st_type_acte, $cjt_intv2->getIdf(), $st_nom_excjt_intv2, $intv2->getIdf(), $st_nom_intv2);
								break;
							default:
								$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, $st_type_acte, $intv2->getIdf(), $st_nom_intv2, $cjt_intv2->getIdf(), $st_nom_excjt_intv2);
						}
		...
		while (!feof($pf)) {
			...
			if (!isset($pa_liste_deces_existants[strval($st_date)][strval($st_nom)][strval($st_prn)])) {
				...
				if ($st_nom_cjt != '') {
					...
					switch ($c_sexe) {
						case 'M':
							$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, utf8_vers_cp1252(LIB_DECES), $defunt->getIdf(), $st_nom, $cjt_defunt->getIdf(), $st_nom_cjt);
							break;
						case 'F':
							$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, utf8_vers_cp1252(LIB_DECES), $cjt_defunt->getIdf(), $st_nom_cjt, $defunt->getIdf(), $st_nom);
							break;
						default:
							$union->ajoute($pi_idf_source, $pi_idf_commune, $i_acte, utf8_vers_cp1252(LIB_DECES), $defunt->getIdf(), $st_nom, $cjt_defunt->getIdf(), $st_nom_cjt);
					}
